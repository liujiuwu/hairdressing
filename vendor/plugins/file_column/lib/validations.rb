      ...
      def validates_file_format_of(*attrs)
        ...
        validates_each(attrs, options) do |record, attr, value|
          unless value.blank?
            ...
            record.errors.add attr, "不合法的图片格式！" unless extensions.include?(value.scan(EXT_REGEXP).flatten.first)
          end
        end
      ...
      def validates_filesize_of(*attrs)  
        ...
        validates_each(attrs, options) do |record, attr, value|
          unless value.blank?
            ...
            record.errors.add attr, "上传的文件太大！" if size > options[:in].last
          end
        end
