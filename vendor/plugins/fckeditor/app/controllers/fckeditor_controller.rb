...
#      }
#    </script>'
    render :text => %Q'<script>if (window.parent.frames[\'frmUpload\']) {window.parent.OnUploadCompleted(#{@errorNumber},\"#{UPLOADED}/#{params[:Type]}/#{@new_file.original_filename}\",\"#{@new_file.original_filename}\",\"\");}</script>'
